...
class controlador_nom_par_otro_pago extends \gamboamartin\nomina\controllers\controlador_nom_par_otro_pago {
    ...
    public function modifica_ajax(bool $header = true, bool $ws = false, array $not_actions = array()): array|string
    {
        ...
        $respuesta = (new nom_par_otro_pago($this->link))->modifica_bd(registro: $registro,id: $this->registro_id);
        if (errores::$error) {
            return $this->retorno_error(mensaje: 'Error al modificar otro pago', data: $respuesta, header: $header,
                ws: $ws);
        }
